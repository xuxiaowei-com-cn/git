...
RUN make
# 编译安装到指定的文件夹下
RUN make DESTDIR=/git-2.35.5 install

# 执行编译安装后的可执行文件
RUN /git-2.35.5/root/bin/git --version
...
# 从第一阶段中复制构建好的可执行文件
COPY --from=git-make /git-2.35.5/root/bin/ /usr/bin/
COPY --from=git-make /git-2.35.5/root/libexec/ /usr/libexec/
COPY --from=git-make /git-2.35.5/root/share/ /usr/share/
